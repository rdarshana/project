...
class DancingGroupRegistrationController extends CI_Controller {

    function __construct() {
        parent::__construct();
        $this->load->database();
        $this->load->library('image_lib');
        $this->load->model("dancingGroupRegistrationModel");
    }
//get the registration no of the dancing group
    public function index() {
        $id = $this->dancingGroupRegistrationModel->getRegistrationNumber();
        $reg_no = NULL;
        if ($id) {
            $x = intval($id) + 1;
            $reg_no = "D00" . $x;
        ...
        $this->load->view('Registration/Dancing/dancing_home-view', $data);
    }
//create the dancing group model
    public function createDancingGroup() {
        ...
        if ($gid == "") {
            if ($this->dancingGroupRegistrationModel->insertDancingGroup($datainfo)) {
                echo "success";
            } else {
                echo "error";
            }
            ...
            if ($this->dancingGroupRegistrationModel->updateDancingGroup($gid, $datainfo)) {
                echo "1";
            } else {
                echo "error";
            }
...
//edit dancing group
    public function editDancingGroup($id) {
        ...
        $groupDetail = $this->dancingGroupRegistrationModel->getGroupDetails($id);
        if ($groupDetail) {
...
//view dancing group data
    public function viewDancingGroup() {
        $data['allDetails'] = $this->dancingGroupRegistrationModel->getAllGroupDetails();
        $this->load->view('Registration/Dancing/DancingGroups-View', $data);
    }
    ...
    public function viewDancingGroupDetails($id) {
        ...
        if ($groupDetail) {
            foreach ($groupDetail as $result) {
                ...
                $data["group_details"] = $result->group_details;
                $data["charges"] = $result->charges;
                $data["images"] = $this->dancingGroupRegistrationModel->getImages($result->id);
            }
        }
        $this->load->view('Registration/Dancing/Dancing_ProfileView', $data);
    }

    public function deleteGroup($id) {
        $images = $this->dancingGroupRegistrationModel->getImages($id);
        if ($images) {
            foreach ($images as $img) {
                unlink("img/uploads/new_" . $img->image_name);
                $this->dancingGroupRegistrationModel->deleteData('dancing_group_images', 'id', $img->id);
            }
        }
        if ($this->dancingGroupRegistrationModel->deleteData('dancing_group_details', 'id', $id)) {
            echo "success";
        }
    }
    ...
    public function loadImagesAddView() {
        $data['allDetails'] = $this->dancingGroupRegistrationModel->getAllGroupDetails();
        $this->load->view('Registration/Dancing/Add_Dancing_Images', $data);
    }
        ...
        if (!empty($_FILES['userFiles']['name'])) {
            ...
            for ($i = 0; $i < $filesCount; $i++) {
                ...
                if ($this->upload->do_upload('userFile')) {
                    ...
                    if ($this->dancingGroupRegistrationModel->insertImages($datainfo)) {
                        echo "success";
                    } else {
                        echo "error";
                    }
